...
author:
  - name: Jocelyn Cramer
    email: jocelynmcramer@colostate.edu
format:
  html:
    code-fold: true
    toc: true
...
```{r}
Discharge_Join <- raw_discharge_change |>
  left_join(site_meta, by = "site_no")

nrow(Discharge_Join) == nrow(raw_discharge_change)
sum(is.na(Discharge_Join$huc_cd)) == 0
```
...
```{r}
Discharge_Low_Flow <- Discharge_Roll |>
  filter(Date >= as.Date("1980-01-01"), Date <= as.Date("2010-12-31")) |>
  group_by(site_no, name) |>
  summarize(Low_Flow = quantile(Flow, probs = 0.10, na.rm = TRUE))
```
...
```{r}
Discharge_Thresholds <- Discharge_Roll |>
  left_join(Discharge_Low_Flow, by = c("site_no", "name")) |>
  mutate(below_threshold = Flow_7Day < Low_Flow)
```
...
Sites_WY <- Discharge_WY |>
  ...
  summarize(Discharge_Total = sum(Flow, na.rm = TRUE),
    ...
    Below_Threshold = sum(below_threshold, na.rm = TRUE))
```
...
nrow(WY_Anomaly) == nrow(Sites_WY)
sum(is.na(WY_Anomaly$Baseline_Mean))
```
...
Annual <- Sites_WY |>
  left_join(site_meta |>
      select(site_no, dec_lat_va, dec_long_va), by = "site_no")

nrow(Annual) == nrow(Sites_WY)
sum(is.na(Annual$dec_lat_va))

Annual2 <- Annual |>
  group_by(Water_Year) |>
  summarize(Mean_Long = sum(dec_long_va * Discharge_Total, na.rm = TRUE) /
      sum(Discharge_Total, na.rm = TRUE),
    Mean_Lat = sum(dec_lat_va * Discharge_Total, na.rm = TRUE) /
      sum(Discharge_Total, na.rm = TRUE))
```
...
```{r}
Discharge_Season <- Discharge_WY |>
  mutate(month = month(Date),
    Season = case_when(month %in% 4:6  ~ "Snowmelt",
...
```{r}
Seasons <- Discharge_Season |>
  group_by(site_no, name, Water_Year, Season) |>
  summarize(seasonal_Discharge = sum(Flow, na.rm = TRUE),
...
aug |>
  arrange(.fitted) |>
  select(Water_Year, .fitted, Log_Lees, .resid, Season) |>
  head(1)
```
...
ggplot(aug, aes(x = .resid)) +
  ...
  labs(x = "Residuals", y = "Count", caption = "Offset normal distribution of residuals due to an outlier near 0.6, causing model skew.") +
  theme_minimal()
```
